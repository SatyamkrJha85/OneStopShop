...
import com.one.stop.shop.dao.UsersDao;
import com.one.stop.shop.entities.Message;
...
public class ConfirmPasswordServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String nwpass = request.getParameter("confirm-password");
            String email = request.getParameter("email");

            UsersDao dao = new UsersDao((OracleConnection) ConnectionProvider.getConnection());
            
            if(dao.updatePassword(email, nwpass)){
                Message msg = new Message("Password Changed successfully", "pass-success");
